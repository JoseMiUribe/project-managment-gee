...
(function () {
      ...
      ? "<ul class=\"notas-lista\">" +
        notas
          .map((n) => {
            ...
            const relacionados = n.relacionados || [];
            const relacionadosHtml = relacionados.length
              ? ' <span class="nota-relacionados">🔗 ' + relacionados.map((id) => escapeHtml(id)).join(", ") + "</span>"
              : "";
            return (
              "<li>" +
  ...
  /** Reutiliza el mismo picker de checkboxes que los campos "relacion" del
   *  GEE (ver buildRelacionPicker) para asociar opcionalmente la nota con
   *  riesgos/dependencias/acciones/impedimentos/cambios de alcance ya
   *  existentes — sin campo "relación" propio en un formulario que no es una
   *  fila del GEE, así que se rellena a mano en vez de vía collectRelacionValue
   *  (que da un string separado por comas, aquí se quiere un array). */
  function renderNotaRelacionPicker() {
    const container = qs("#nota-relacion-picker");
    if (!container) return;
    container.innerHTML = "";
    container.appendChild(buildRelacionPicker({ relTipos: ["riesgos", "dependencias", "acciones", "impedimentos", "changelog"] }, ""));
  }

  function collectNotaRelacionados() {
    const container = qs("#nota-relacion-picker");
    if (!container) return [];
  ...
  function setupNuevaNotaForm() {
    ...
    btnCrear.addEventListener("click", async () => {
      ...
      try {
        ...
        const resp = await apiSend("POST", "/api/dailylog", { confirm: true, fecha, autor, texto, relacionados });
        state = resp.snapshot;
        renderAll();
