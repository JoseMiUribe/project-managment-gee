...
**Creación de issues (épicas e historias) vía el conector de la sesión: confirmada en la práctica.** Una versión anterior de este skill ya creó épicas e historias reales en un Jira real usando exactamente este mecanismo (conector MCP de la sesión, sin tocar el token) — la operación en sí (crear issues tipo Épica/Historia) funciona. Lo que sí sigue cambiando entre versiones no es la conexión, sino el **contenido** de las historias generadas (ver `mejoras-pendientes.md`, entradas 2026-07-07 y 2026-07-10) — ese es el punto que se está calibrando con feedback real del equipo, no la integración con Jira.

`prompts/paso-3/crear-sprints-jira.md` (creación/edición de objetos Sprint, no issues) sigue **sin verificar** — no está confirmado que el conector de la sesión soporte el API Agile de Sprints (fechas, goal) de la misma forma que soporta issues. Trátalo como beta hasta la primera ejecución real, y registra el resultado en `mejoras-pendientes.md`.
